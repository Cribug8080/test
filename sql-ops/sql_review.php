		...
		array_push($dml_parm,$parmArr[0]);
            ...
            while($row = mysql_fetch_array($result)){
                  ...
                  if($record_rows<=50000){
						 ...
						 array_push($dml_big_array,"${parmArr[1]} 表 where条件字段，扫描影响的行数是：${record_rows}行，超过50000行请联系DBA执行!</br></br>联系不上请打电话---贺春旸：13581549970（钱站库）；王加富：18515668401（凡普信库）</br></br>");
                         //$up++;
                  }
            }
            mysql_close($con1);
				...
				$prompt_message[]='警告！没有where条件，delete会全表删除，禁止执行！';
            		...
            		while($row = mysqli_fetch_array($result)){
				        ...
				        echo '<big><font color="#FF0000">'.$parmArr[2].'表 where条件字段，扫描影响的行数是：'.$record_rows.' 行，超过50000行请联系DBA执行!!!</font></big></br>';
                                        array_push($dml_big_array,"${parmArr[2]} 表 where条件字段，扫描影响的行数是：${record_rows}行，超过50000行请联系DBA执行!</br></
br>联系>不上请打电话---贺春旸：13581549970（钱站库）；王加富：18515668401（凡普信库）</br></br>");
					//$de++;
                                        //exit;
                  		}
            		}	
	    ...
	    array_push($dml_parm,$parmArr[0]);
			...
			$row = mysqli_fetch_array($result);
				   ...
				   if($record_rows<=1500000){
					    ...
					    $is_small=1;
				   	    ...
				   	    array_push($is_big_array,"$parmArr[2] 表记录是：${record_rows}行，表太大请联系DBA在业务低峰期执行!</br></br>联系不上请打电话---贺春旸：13581549970（钱站库）；王加富：18515668401（凡普信库）</br></br>");
            ...
            if(in_array('drop',$parmArr)){
                   if(!preg_match('/drop.*index|drop.*foreign/i',$multi_sql[$x])){
                        echo "<big><font color=\"#FF0000\">警告！你要对$parmArr[2]表删除字段，数据会存在丢失的风险，请三思！！！请先私下联系下大数据团队，看是否有问题。</font></big></br>";
                        exit;
                   }
            }
...
$sql_replace=$parm_post;
$conn=mysqli_connect("192.168.148.9","admin","wdhcy159753","sql_db");
$ops_time="NOW()";
